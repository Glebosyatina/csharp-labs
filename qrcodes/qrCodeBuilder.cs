...
internal static class QrCodeBuilder
{
    ...
    /// Magic
    /// </summary>
    private static char Magic(byte[] a) => (a[0], a[1]) switch
    {
        (0, 0) => ' ',
        (0, 1) => '▄',
        (1, 0) => '▀',
   ...
   POSITION_DETECTION + BORDER + 1);
    ...
    private static string Magic(string text, ref EncodingMode? codeType)
    {
        ...
        var sb = codeType switch
        {
         ...
         Magic(text, [8]),
            _ => throw new
           NotSupportedException("Current type of encoding not supported!"),
        };
        return
...
,18,16,19,21,25,25]},
    ...
    private static readonly Dictionary<byte, byte[]> _correctionLevelGeneratingPolynomial
   = new()
    {
...
QR.V20), 5352}, {(EccLevel.L, QR.V20), 6888},
    ...
    /// Magic
    /// </summary>
    private static (string a, EccLevel b, QR c) Magic(string a, string b, EncodingMode c,
    ...
    /// Format information
    /// </summary>
    private static readonly Dictionary<(EccLevel correctionLevel, Mask maskNum), string> _masksAndCorrectionLevel =
    new()
    {
...
b), (offset + 7, b)];
    ...
    /// </summary>
    private static (int a, List<byte[]> b)
    Magic(this (int a, List<byte[]> b) a, double b)
    {
        var cntTotal = 0.0;
        for (int x = BORDER; x < a.b.Count -
       BORDER; x++)
    ...
    {
        var res = Enumerable.Range(0, 8).Select(maskNumber =>
