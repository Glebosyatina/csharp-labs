...
internal static class QrCodeBuilder
{
    ...
    private static string getStringFromListBytes(this List<byte[]> a, bool b)
    {
        ...
        for (int row = 0; row < length; row += 2)
        {
            for (int column = 0; column < a[0].Length; column++)
            {
                ...
                var c = b ? Magic((scanModule1, scanModule2)) : Magic([scanModule1, scanModule2]);
                sb.Append(c);
            }
            sb.AppendLine();
        }
    ...
    private static string defineTypeOfInformationAndEncodeIt(string text, ref EncodingMode? codeType)
    {
        ...
        if (!codeType.HasValue)
        {
           ...
           char.IsDigit(x)))
            {
                codeType = EncodingMode.Numeric; //цифровой, каждые 3 символа пакуются в 10 бит, затем склеиваются
            }
            else if (text.All(x => letterNumberArray.Any(y => x == y)))
            {
                codeType = EncodingMode.AlphaNumeric; //буквенно цифровой, разбиваются на пары сиволов a * 45 + b, и так склеиваются, последний если без пары записывается 6 битным числом и добавляется к остальным
            }
            else
            {
                codeType = EncodingMode.Binary;     //байты напрямую пишутся друг за другом
            }
        }
        var sb = codeType switch
        {
            //в зависимости от определенного типа по разному кодируем данные и пишем их встринг билдер
            EncodingMode.Numeric => NumericCoding(text, (10, 7, 4)),
            EncodingMode.AlphaNumeric => AlphaNumericCoding(text.ToUpper(), 0, 45, 11),
            EncodingMode.Binary => BytesCoding(text, [8]),
    ...
    /// <summary>
    /// Буквенно цифровое представление, индексы соотвествуют коду для кодирования
    /// </summary>
    ...
    /// <summary>
    /// кодирование числа (int) в побитовую строку, нужного размера 
    /// </summary>
    private static void Encode(StringBuilder sb, int num, byte lengthType)
    {
        var str = Convert.ToString(num, 2); //перевели строку в двоичное представление
        sb.Append(str.PadLeft(lengthType, '0')); //дополнили до нужной длины слева
    }
    /// <summary>
    /// буквенно цифровое кодирование
    /// </summary>
    private static StringBuilder AlphaNumericCoding(string input_str, int b, int c, byte leghtBits)
    {
        StringBuilder sb = new();

        while (b <= input_str.Length - 2)
        {
            //ищем индексы входных символов в массиве чаров
            var idx1 = Array.IndexOf(letterNumberArray, input_str[b]);
            var idx2 = Array.IndexOf(letterNumberArray, input_str[b + 1]);

            if (idx1 == -1) throw new InvalidDataException($"Not supported character {input_str[b]}!");
            if (idx2 == -1) throw new InvalidDataException($"Not supported character {input_str[b + 1]}!");

            var number = idx1 * c + idx2; //получаем число, затем переводим его в 11битное представление
            Encode(sb, number, leghtBits); //d == 11, закодировали, добавили к строке 
            b += 2;//перешли к следующей паре символов
        }
        if (input_str.Length % 2 == 1)  //когда длина строки нечетная, то есть нужно будет последний символ добавлять без пары
        {
            var number = Array.IndexOf(letterNumberArray, input_str[^1]);

            if (number == -1) throw new InvalidDataException($"Not supported character {input_str[^1]}!");

            Encode(sb, number, 6); //d == 6, закодировали, добавили к строке 
        }
        return sb;
    }
    ...
    private static StringBuilder NumericCoding(string text, (byte three_digits, byte two_digits, byte one_digit) len_bits)
    {
        StringBuilder sb = new();
        var pos = 0;
        while (pos <= text.Length - 3)//кодируем все полные тройки
        {
            var number = Convert.ToInt32(text.Substring(pos, 3));//конвертим в число 
            Encode(sb, number, len_bits.three_digits);//кодируем до 10 бит, a=10
            pos += 3;
        }
        if (text.Length % 3 == 2) //если осталось два символа кодируем их 7 битами
        {
            var number = Convert.ToInt32(text.Substring(pos, 2));
            Encode(sb, number, len_bits.two_digits);
        }
        else if (text.Length % 3 == 1) //если остался один символ кодируем его 4 битами
        {
            var number = Convert.ToInt32(text.Substring(pos, 1));
            Encode(sb, number, len_bits.one_digit);
        }
        return sb;
    }
    ...
    /// </summary>
    private static StringBuilder BytesCoding(string text, byte[] b)
    {
        StringBuilder sb = new();
        var bytes = Encoding.UTF8.GetBytes(text);//перевели строку в массив байт
        foreach (var bt in bytes)
        {
            Encode(sb, Convert.ToInt32(bt), b[0]); //каждый байт кодируем и добавляем к стрингбилдеру
        }
        return sb;
    }
    /// <summary>
    /// мапа хранящяя 4 бита в зависимости от вида кодирования
    /// </summary>
    ...
    /// <summary>
    /// длина поля количества данных в зависимости от типа данных и версии qr кода
    /// </summary>
    private static byte GetLengthFieldStoreAmountOfData(EncodingMode? encMode, QR qrVersion)
    {
        return ((int)qrVersion, encMode) switch
        {
    ...
    /// <summary>
    /// считаем длину исходных данных
    /// </summary>
    private static string GetDataLength(EncodingMode? encMode, QR qrVersion, string inputStr)
    {
        var length = encMode switch
        {
            EncodingMode.Binary => Encoding.UTF8.GetBytes(inputStr).Length, //если бинарный режим кодирования переводим в массив байт и считаем длину, иначе просто возвращаем длину
            _ => inputStr.Length,
        };
    ...
    /// <summary>
    /// Пишем с qrcode информацию о алгоритме кодирования и длине исходных данных
    /// </summary>
    ...
    private static string GetMethodCodingAndLengthData(EncodingMode encMode, QR qrVer, string inputText, Dictionary<EncodingMode, string> mapEncodingMode)
    {
